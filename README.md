...
- **RAG 知识库**：多格式文档上传（txt/pdf/md/pptx/docx）与检索增强问答
- **间隔重复回顾**：艾宾浩斯遗忘曲线算法，对抗遗忘
- **AI 写作辅助**：联机补全、续写/扩写/摘要、关联笔记推荐

系统支持会话持久化（MySQL）、向量检索（ChromaDB）、JWT 身份认证与用户作用域约束，前端采用 Vue 3 构建桌面端侧边栏布局，支持深浅主题切换。
...
- **🤖 AI 写作助手**：续写、扩写、摘要生成，通过 SSE 返回生成结果
...
- **⛑️ 身份边界**：关键检索与资源接口实施用户作用域约束并覆盖离线负向测试；生产隔离仍需真实存储集成验证
...
```bash
git clone https://github.com/joy-toy5/RAGNote.git
cd RAG-Note
```
...
# 原始上传文件的不可变 content-addressed 存储目录
RAG_BLOB_STORE_PATH=data/blobs
...
# ==================== JWT 身份验证配置 ====================
# 必须与 Django 的 JWT_SECRET_KEY 一致；生产环境使用独立生成的强密钥
SECRET_KEY=YOUR_SHARED_JWT_SECRET_KEY
ALGORITHM=HS256
```
...
| 用户服务 | `cd DjangoUserService && DJANGO_ENV=development uv run python manage.py runserver 8001` | 8001 |
...
| 技术 | 说明 |
|------|------|
| Vue 3 | 现代化前端框架（Composition API + `<script setup>`） |
| Vite | 极速构建工具 |
| Vant 4 | 轻量 UI 组件库 |
| bytemd | Markdown 编辑器（Web Component） |
| Vue Router | 路由管理（路由守卫 + JWT 校验） |
| Pinia | 状态管理（user/session/theme/language） |
| Vue i18n | 国际化（中/英） |
| Axios | HTTP 客户端 |
| highlight.js | 代码语法高亮 |
| dompurify | HTML 安全过滤 |
...
│   ├── src/
│   │   ├── components/          # 通用组件
│   │   │   ├── AppSidebar.vue        # 桌面侧边栏（导航 + 会话列表 + 用户信息）
│   │   │   ├── SettingsPopover.vue   # 设置浮窗菜单
...
│   │   │   └── Profile.vue / Settings.vue / AboutUs.vue
│   │   ├── router/index.js     # 路由配置（keepAlive + 无侧边栏布局标记）
│   │   ├── store/              # Pinia 状态管理
│   │   ├── i18n/               # 国际化（中/英）
