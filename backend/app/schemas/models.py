...
class ReorderRequest(BaseModel):
    ...
    def validate_document_sizes(self) -> "ReorderRequest":
        if any(not document or len(document) > 12000 for document in self.documents):
            raise ValueError("每个待重排文档长度必须在 1 到 12000 个字符之间")
        if sum(len(document) for document in self.documents) > 100000:
            raise ValueError("待重排文档总长度不能超过 100000 个字符")
        return self
