...
class AgentFactory:
    ...
    def create_agent_executor(
        ...
        kwargs.setdefault("max_execution_time", 60)
...
async def get_agent_response(
    ...
    """在可信用户上下文中执行一次 Agent 请求。"""
    with bind_tool_context(ToolContext(user_id=user_id)):
        return await _get_agent_response(query, history, custom_tools, **kwargs)
    ...
    async def run_agent():
        ...
        try:
            ...
            with bind_tool_context(
                ToolContext(user_id=user_id, thinking_callback=thinking_callback)
            ):
                async for chunk in agent_executor.astream({
                    "input": query,
                    "chat_history": chat_history,
