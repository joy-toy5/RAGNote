<template>
  <div class="knowledgebase-container">
    ...
    <div class="knowledgebase-content">
      ...
      <div v-if="uploading || uploadProgressList.length > 0" class="upload-progress">
        ...
        <div v-if="uploadComplete" class="upload-result">
          <van-icon
            :name="failedCount > 0 ? 'warning-o' : 'success'"
            size="32"
            :color="failedCount > 0 ? '#ee0a24' : '#07c160'"
          />
          <p>{{ failedCount > 0 ? $t('knowledgebase.uploadFinishedWithErrors') : $t('knowledgebase.uploadComplete') }}</p>
...
const addFiles = (files) => {
  ...
  let totalSize = getSelectedTotalSize();

  files.forEach((file) => {
    if (file.size > MAX_FILE_SIZE) {
      rejectedMessages.push(t('knowledgebase.fileTooLarge', {
        filename: file.name,
    ...
    if (totalSize + file.size > MAX_FOLDER_SIZE) {
      rejectedMessages.push(t('knowledgebase.totalSizeTooLarge', {
        maxSize: formatFileSize(MAX_FOLDER_SIZE)
      }));
      return;
    }
  ...
  if (rejectedMessages.length > 0) {
    showUploadToast(rejectedMessages[0]);
  }
};

const validateSelectedFiles = () => {
  const oversizedFile = selectedFiles.value.find(file => file.size > MAX_FILE_SIZE);
  if (oversizedFile) {
    return t('knowledgebase.fileTooLarge', {
      filename: oversizedFile.name,
      maxSize: formatFileSize(MAX_FILE_SIZE)
    });
  }

  if (getSelectedTotalSize() > MAX_FOLDER_SIZE) {
    return t('knowledgebase.totalSizeTooLarge', {
      maxSize: formatFileSize(MAX_FOLDER_SIZE)
    });
  }
...
const syncUploadCounts = () => {
  successCount.value = uploadProgressList.value.filter(item => item.status === 'completed').length;
  failedCount.value = uploadProgressList.value.filter(item => item.status === 'failed').length;
};

const formatEventMessage = (message, errorMessage) => {
  if (!message && !errorMessage) return t('knowledgebase.uploadError');
  if (!errorMessage || errorMessage === message) return message || errorMessage;
  if (!message) return errorMessage;
  return `${message}: ${errorMessage}`;
};

const failPendingUploads = (message) => {
...
const finalizePendingUploads = () => {
  failPendingUploads(t('knowledgebase.uploadInterrupted'));
};

const getResponseErrorMessage = async (response) => {
  if (response.status === 413) {
    return t('knowledgebase.requestTooLarge');
  }

  try {
    const result = await response.json();
    return result.detail || result.message || t('knowledgebase.uploadError');
  } catch (error) {
    return t('knowledgebase.uploadError');
  }
};
...
const uploadFiles = async () => {
  ...
  const validationMessage = validateSelectedFiles();
  if (validationMessage) {
    showUploadToast(validationMessage);
    return;
  }
  ...
  try {
    ...
    const message = error.message || t('knowledgebase.uploadError');
    showUploadToast(message);
    failPendingUploads(message);
  } finally {
...
const parseEvent = (event) => {
  ...
  try {
    ...
    const { event_type, filename, message, progress, error_message } = eventData;
    const displayMessage = formatEventMessage(message, error_message);

    if (filename) {
      const index = uploadProgressList.value.findIndex(p => p.filename === filename);
      if (index !== -1) {
        uploadProgressList.value[index].message = displayMessage;
        
        if (event_type === 'completed') {
          uploadProgressList.value[index].status = 'completed';
          uploadProgressList.value[index].percentage = 100;
        } else if (event_type === 'error') {
          uploadProgressList.value[index].status = 'failed';
          uploadProgressList.value[index].percentage = progress ?? null;
        } else if (['processing', 'slicing_completed', 'writing'].includes(event_type)) {
      ...
      showUploadToast(displayMessage);
      failPendingUploads(displayMessage);
